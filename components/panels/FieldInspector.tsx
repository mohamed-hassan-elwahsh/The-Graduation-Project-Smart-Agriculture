import { C, CROP_COLORS } from '@/lib/constants';
import type { Field, Lang, Translation } from '@/types';

interface Props { t: Translation; lang: Lang; field: Field | null; }

export default function FieldInspector({ t, lang, field }: Props) {
    return (
        <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, display: 'flex', flexDirection: 'column' }}>
            ...
            {!field ? (
                ...
                <div style={{ padding: 12 }}>
                    ...
                    <button style={{ width: '100%', padding: 6, background: 'rgba(14,165,233,.07)', border: '1px solid rgba(14,165,233,.22)', borderRadius: 5, color: C.sky, fontSize: 11, cursor: 'pointer', fontFamily: 'inherit' }}>
                        {t.viewBtn}
                    </button>
                </div>
            )}
